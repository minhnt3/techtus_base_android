...
enum class WindowWidth { Small, Normal, Large }

@Composable
fun Activity.rememberWindowWidth(): WindowWidth {
    val windowSize = rememberWindowSize()
    val windowDpSize = with(LocalDensity.current) {
        windowSize.toDpSize()
    }
    return when {
        windowDpSize.width < 384.dp -> WindowWidth.Small
        windowDpSize.width < 600.dp -> WindowWidth.Normal
        else -> WindowWidth.Large
    }
}
